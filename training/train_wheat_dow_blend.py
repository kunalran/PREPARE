...
def parse_args() -> Config:
    parser = argparse.ArgumentParser(
        description=(
            "Train a wheat-specific daily forecasting model using reference "
    ...
    parser.add_argument(
        "--data-path",
        type=Path,
        default=Path("final_data/agmarknet_wheat_data_final.csv"),
        help="Path to the wheat daily CSV in final_data.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("models/wheat_dow_blend"),
    ...
    parser.add_argument(
        ...
        type=int,
        default=30,
        help="Drop mandi series with fewer observed wheat prices than this.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--anchor-col",
        choices=("price_roll_mean_7", "price_roll_mean_14", "price_roll_mean_28"),
...
def impute_dow_ratio(df: pd.DataFrame, col: str) -> pd.Series:
    ...
    This is intentionally centered because the user asked for the wheat
...
def filter_series(frame: pd.DataFrame, min_series_observations: int) -> pd.DataFrame:
    ...
    print(
        f"Kept {len(valid_series):,} wheat series with at least "
...
def load_and_engineer_wheat(path: Path, horizons: list[int]) -> pd.DataFrame:
    usecols = [
        "State",
        "District",
        "Market",
    ...
    print("Applying reference DOW-ratio imputation for wheat price and arrival ...")
...
def train_models(data: pd.DataFrame, config: Config) -> list[dict]:
    ...
    for horizon in config.horizons:
        ...
        if horizon_frame.empty:
            print(f"Skipping wheat {horizon}d: no rows with non-null targets.")
            continue
        ...
        if train_frame.empty or val_frame.empty:
            print(f"Skipping wheat {horizon}d: train or validation split is empty.")
            continue
        ...
        if train_frame.empty or val_frame.empty:
            print(f"Skipping wheat {horizon}d: no rows with a usable anchor.")
            continue
        ...
        print(
            f"Training wheat {horizon}d around {config.anchor_col} with "
        ...
        model_path = config.output_dir / f"wheat_model_{horizon}d.joblib"
        joblib.dump(artifact, model_path)
        print(
            f"  wheat {horizon}d: anchor_r2={row['anchor_r2']:.4f}, "
...
def main() -> None:
    ...
    if not config.data_path.exists():
        raise FileNotFoundError(f"Wheat data file not found: {config.data_path}")

    print(f"Loading wheat daily data from {config.data_path} ...")
    data = load_and_engineer_wheat(config.data_path, config.horizons)
    ...
    summary_path = config.output_dir / "wheat_training_summary.json"
    summary_path.write_text(json.dumps(summary, indent=2))
    print(f"Saved wheat summary to {summary_path}")
